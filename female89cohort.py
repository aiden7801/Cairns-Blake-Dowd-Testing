...
from Testing.baseline import insurance_type
...
insurance_type = 0
insurance_type = int(input("Enter 0 to skip, 1 for Term life Insurance, 2 for Pure Endowment Insurance, 3 for Endowment Insurance or 4 for Whole Life : "))
if insurance_type==1 or insurance_type==2 or insurance_type==3 or insurance_type==4:
...
def whole_life_premium(age, forecast_start_year, interest_rate, benefit):
    apv_benefit = 0
    apv_premiums = 0
    survival_prob = 1.0

    for t in range(1, (89 - age) + 1):
        current_age = age + t - 1
        current_year = forecast_start_year + t - 1
...
#plt.legend()
#plt.title('Female k²_t Observed vs Forecasted')
#plt.xlabel('Year')
#plt.ylabel('k²_t')
#plt.show()
...
plt.colorbar()
plt.title('Females CBD Residuals')
plt.xlabel('Year')
plt.ylabel('Age')
plt.xticks(range(0, len(logit_qx.columns), 5),
           logit_qx.columns[::5], rotation=90)
plt.yticks(range(len(logit_qx.index)), logit_qx.index)
plt.gca().invert_yaxis()
plt.tight_layout()
plt.show()
cohort_effects = {}

for year in residuals_check.columns:
    for age in residuals_check.index:
        birth_year = year - age
        value = residuals_check.loc[age, year]
        if birth_year not in cohort_effects:
            cohort_effects[birth_year] = []
        cohort_effects[birth_year].append(value)
cohort_means = {by: np.mean(vals)
                for by, vals in cohort_effects.items()}

overall_mean = np.mean(list(cohort_means.values()))

w = {by: val - overall_mean
     for by, val in cohort_means.items()}


cohort_adjusted = logit_qx.copy()

for year in logit_qx.columns:
    for age in logit_qx.index:
        birth_year = year - age
        if birth_year in w:
            cohort_adjusted.loc[age, year] += w[birth_year]




original_errors = (logit_qx - fitted).values.flatten()
mae_original = np.mean(np.abs(original_errors))
rmse_original = np.sqrt(np.mean(original_errors**2))


fitted_cohort = fitted.copy()
for year in fitted_cohort.columns:
    for age in fitted_cohort.index:
        birth_year = year - age
        if birth_year in w:
            fitted_cohort.loc[age, year] += w[birth_year]


cohort_errors = (logit_qx - fitted_cohort).values.flatten()
mae_cohort = np.mean(np.abs(cohort_errors))
rmse_cohort = np.sqrt(np.mean(cohort_errors**2))

print(f"Original  | MAE: {mae_original:.6f} | RMSE: {rmse_original:.6f}")
print(f"Cohort    | MAE: {mae_cohort:.6f} | RMSE: {rmse_cohort:.6f}")
...
if insurance_type==1:
    ...
    premium, apv_b, apv_p = whole_life_premium(age_input, year_input, rate_input, benefit_input)
